...
import { BASE_URL, isLoggedIn, addToCart } from "../../utils/api";
...
export default function BraceletGrid() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only bracelet products and transform data
        let braceletProducts = data
          .filter(product => product.category && product.category.toLowerCase() === "bracelet")
          .map(product => ({
            ...product,
            id: product.id,
            name: product.name,
  ...
  // HOVER LOGIC
  // Track hovered product id and show second image if hovered and available
  // Add hoveredProductId state and logic
  // Only change: add onMouseEnter/onMouseLeave and show second image if hovered
