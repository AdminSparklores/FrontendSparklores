...
export default function ProductGrid() {
  ...
  // Fetch gift sets from API and filter by label "forHer"
  useEffect(() => {
    const fetchGiftSets = async () => {
      try {
        ...
        // Only keep those with "label": "forHer" (case-insensitive)
        const filtered = data
          .filter((item) => item.label && item.label.toLowerCase() === "forher")
  ...
  // If there are no giftSets with label: "forHer", show Coming Soon
